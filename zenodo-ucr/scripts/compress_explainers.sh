...
# 4. A phar_values archive containing: pvts.pickle, pvtr.pickle, phar_metadata.json, phar_trials_log.jsonl
# 5. A train_and_test archive containing: trainX.pickle, trainy.pickle, testX.pickle, testy.pickle
#
# The archives are placed in separate subdirectories in the current directory:
# shap, lime, anchor, phar, train_test.
#
...
#   shap_values.zip, lime_values.zip, anchor_values.zip, phar_values.zip, or train_and_test.zip.
#
...
mkdir -p shap lime anchor phar train_test
...
for dataset in multivariate univariate; do
    ...
    for series_dir in "$dataset_dir"/*; do
        if [ -d "$series_dir" ]; then
            ...
            # Compress phar values (pvts.pickle, pvtr.pickle, phar_metadata.json, phar_trials_log.jsonl)
            if [ -f "$series_dir/pvts.pickle" ] && [ -f "$series_dir/pvtr.pickle" ] && [ -f "$series_dir/phar_metadata.json" ] && [ -f "$series_dir/phar_trials_log.jsonl" ]; then
                archive_name="${dataset}_${series_name}_phar_values.zip"
                echo "  Creating phar archive: $archive_name"
                zip -j "phar/$archive_name" "$series_dir/pvts.pickle" "$series_dir/pvtr.pickle" "$series_dir/phar_metadata.json" "$series_dir/phar_trials_log.jsonl"
            else
                echo "  Phar files missing for $series_name. Skipping phar archive."
            fi
...
echo "Compression complete. Archives are available in the subdirectories: shap, lime, anchor, phar, train_test."